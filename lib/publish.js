...
function compileDocument ({ source, recipe, format }) {
  ...
  const procPandoc = spawn(
    pandocCmd,
    {
      cwd: recipeFolder,
      stdio: [procCat.stdout, process.stdout, process.stderr],
      shell: true
    }
  );

  procCat.on('error', err => console.log(err));
  procPandoc.on('error', err => console.log(err));

  procPandoc.on('exit', (code) => {
    if (code === 0) {
      console.log('ok');
    } else {
      console.log('fail');
