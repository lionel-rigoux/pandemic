...
function compileDocument ({ source, recipe, format }) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (compiler.status) {
      reject(compiler.stderr.toString());
    }
    // pipe all the pre processing hooks
    (instructions.prehooks || []).forEach((hook) => {
      const [cmd, ...opts] = hook.split(/\s+/);
      compiler = spawnSync(
        cmd, opts,
        {
          cwd: recipeFolder,
          input: compiler.stdout,
          shell: true
        }
      );
      if (compiler.status) {
    ...
    // pipe to the actual pandoc command
    compiler = spawnSync(
      pandocCmd,
      {
        cwd: recipeFolder,
