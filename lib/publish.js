...
function compileDocument ({
  ...
  return new Promise((resolve, reject) => {
    ...
    if (instructions.template) {
      if (targetExt === 'docx') {
        pandocCmd += ` --reference-doc="${path.join(
          recipeFolder,
    ...
    pandocCmd += ` -V sourcedir="${sourceDir}"`;
    ...
    // Prepare all the preprocessing steps
    // =========================================================================

    // check for mustache: front-matter > default json > none
    const defaultMustache = 'results.json';
    let mustache = [];
    if (
      !yamlOptions.mustache && // pandemic-mustache will use it
      fs.existsSync(path.resolve(sourceDir, defaultMustache))
    ) {
      mustache = [defaultMustache];
    }
    // start with preprocessing
    const defaultPreprocessing = [
      `pandemic-include`,
      `pandemic-mustache ${mustache.join(' ')}`
    ];
    const pipeline = instructions.preprocessing || defaultPreprocessing;
